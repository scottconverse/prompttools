...
from prompttest.models import AssertionResult, AssertionType, PromptTestReport, PromptTestStatus
from prompttest.reporter import format_json, format_junit, format_text


@pytest.fixture
def sample_report() -> PromptTestReport:
...
@pytest.fixture
def empty_report() -> PromptTestReport:
...
class TestFormatText:
    ...
    def test_contains_suite_name(self, sample_report: PromptTestReport) -> None:
        output = format_text(sample_report)
        assert "greeting-tests" in output

    def test_contains_pass_indicator(self, sample_report: PromptTestReport) -> None:
        output = format_text(sample_report)
        assert "PASS" in output

    def test_contains_fail_indicator(self, sample_report: PromptTestReport) -> None:
        output = format_text(sample_report)
        assert "FAIL" in output

    def test_contains_error_indicator(self, sample_report: PromptTestReport) -> None:
        output = format_text(sample_report)
        assert "ERR" in output

    def test_contains_skip_indicator(self, sample_report: PromptTestReport) -> None:
        output = format_text(sample_report)
        assert "SKIP" in output

    def test_contains_summary(self, sample_report: PromptTestReport) -> None:
        output = format_text(sample_report)
    ...
    def test_contains_failure_message(self, sample_report: PromptTestReport) -> None:
        output = format_text(sample_report)
        assert "exceeds limit" in output

    def test_empty_report(self, empty_report: PromptTestReport) -> None:
...
class TestFormatJson:
    ...
    def test_valid_json(self, sample_report: PromptTestReport) -> None:
        output = format_json(sample_report)
        data = json.loads(output)
        assert isinstance(data, dict)

    def test_json_fields(self, sample_report: PromptTestReport) -> None:
        output = format_json(sample_report)
        data = json.loads(output)
        assert data["total"] == 5
    ...
    def test_json_suites(self, sample_report: PromptTestReport) -> None:
        output = format_json(sample_report)
        data = json.loads(output)
    ...
    def test_empty_json(self, empty_report: PromptTestReport) -> None:
...
class TestFormatJunit:
    ...
    def test_valid_xml(self, sample_report: PromptTestReport) -> None:
        output = format_junit(sample_report)
        root = ET.fromstring(output)
        assert root.tag == "testsuites"

    def test_testsuites_attributes(self, sample_report: PromptTestReport) -> None:
        output = format_junit(sample_report)
        root = ET.fromstring(output)
    ...
    def test_testsuite_element(self, sample_report: PromptTestReport) -> None:
        output = format_junit(sample_report)
        root = ET.fromstring(output)
    ...
    def test_testcase_elements(self, sample_report: PromptTestReport) -> None:
        output = format_junit(sample_report)
        root = ET.fromstring(output)
        testcases = root.findall(".//testcase")
        assert len(testcases) == 5

    def test_failure_element(self, sample_report: PromptTestReport) -> None:
        output = format_junit(sample_report)
        root = ET.fromstring(output)
    ...
    def test_error_element(self, sample_report: PromptTestReport) -> None:
        output = format_junit(sample_report)
        root = ET.fromstring(output)
        errors = root.findall(".//error")
        assert len(errors) == 1

    def test_skipped_element(self, sample_report: PromptTestReport) -> None:
        output = format_junit(sample_report)
        root = ET.fromstring(output)
        skipped = root.findall(".//skipped")
        assert len(skipped) == 1

    def test_xml_declaration(self, sample_report: PromptTestReport) -> None:
        output = format_junit(sample_report)
        assert output.startswith('<?xml version="1.0"')

    def test_empty_junit(self, empty_report: PromptTestReport) -> None:
